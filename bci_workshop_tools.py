...
def compute_feature_vector(eegdata, fs):
    ...
    feature_vector = np.concatenate((meanDelta, meanTheta, meanAlpha,
                                     meanBeta, meanAlpha/meanBeta), axis=0)
...
def train_classifier(feature_matrix_0, feature_matrix_1, algorithm='SVM'):
    ...
    X = np.loadtxt('Xdata.npy')
    mu_ft = np.loadtxt('muft.npy')
    std_ft = np.loadtxt('stdft.npy')"""
    ...
    """ data = loadmat('Session_01_pod_01_player_01.mat')
    features_all_t = np.concatenate((fmt0,fmt1),axis = 0)
    mu_ft_t = np.mean(features_all_t, axis=0)
    std_ft_t = np.std(features_all_t, axis=0)

    X_t = (features_all_t - mu_ft_t) / std_ft_t
    class0_t = np.zeros(data['cond_eeg'][0][0],1)
    class1_t = np.ones(data['cond_eeg'][0][1],1)
    y_t = np.concatenate((Class0_t,class1_t),axis = 0)"""
...
def get_feature_names(ch_names):
    ...
    Returns:
        (list): feature names
    """
    bands = ['delta', 'theta', 'alpha', 'beta','theta/beta']
